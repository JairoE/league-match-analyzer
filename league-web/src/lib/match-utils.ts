...
export function getMatchId(match: MatchSummary): string | null {
  const candidate =
    match.matchId ?? match.match_id ?? match.id ?? (match as { matchID?: string }).matchID;
...
export function getParticipantForUser(
  ...
  const targetPuuid = user?.puuid;
  const targetName =
    user?.summonerName ?? user?.summoner_name ?? user?.gameName ?? undefined;

  if (targetPuuid) {
    const match = participants.find((participant) => participant.puuid === targetPuuid);
  ...
  if (targetName) {
    ...
    const match = participants.find((participant) => {
      const candidate =
        participant.summonerName ?? participant.gameName ?? participant.name ?? "";
...
export function getCsPerMinute(participant: Participant | null): number {
  if (!participant) return 0;
  const total = (participant.totalMinionsKilled ?? 0) + (participant.neutralMinionsKilled ?? 0);

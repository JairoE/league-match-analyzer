# App State — Historical Changelog Archive

This file holds the full session-by-session change history that used to live inline in
`docs/app_state.md`. It is **not loaded by default** by agents or the `/UPDATE_APP_STATE`
command — `docs/app_state.md` now carries only current-state (architecture, blockers, next
steps), rewritten in place each session rather than appended to.

Read this file only when you need archaeology on *why* something is shaped the way it is and
`git log` / PR descriptions aren't enough. For most purposes, `git log --oneline` and merged PRs
are the source of truth for history — this file predates that discipline and is kept for
continuity on entries where the narrative adds context git alone doesn't carry.

Newest entries are at the top, matching the original document order.
...
## Recent Changes (2026-03-04, Railway Deploy Hardening + Champion KDA Chart — duplicate heading note)

_(See the 2026-03-04 entry above; Railway hardening and the KDA chart landed in the same session.)_

---

## Next Recommended Steps (superseded — see "Next Steps" in docs/app_state.md)
...
1. **LLM Pipeline Step 6 — Compare**: Build comparison logic (rank summoner choices vs optimal alternatives by ΔW). — **DONE**, see 2026-03-17 entry.
2. **LLM Pipeline Step 7 — LLM Prompt**: Build gap analysis payload and submit to LLM for recommendations. Populate `llm_analysis` table. — **DONE**, see 2026-03-18 entries.
...
5. **Live Game integration** — **DONE**, see 2026-03-06 "Live game" entry.
6. **Consider server-side queue filtering** — current tab filtering is client-side. Still open as of 2026-07-13.
7. **Implement vector embeddings** — **DONE**, see the 2026-06-01 RAG few-shot retrieval entry.
